...
//Returns values from a specified column of a double array
...
//Draws the axes and buttons with all the labels
void axes() {
...
//Checks if mouse is over a line within a certain accuracy
void check_hover() {
  for (int j = 0; j < values.length; ++j) {
    for (int i = 0; i < values[j].length-1; ++i) {
      int cur_value = values[j][i];
...
//If mouse is moved, rechecks if mouse is hovering over line
void mouseMoved() {
  for (int i = 0; i < line_hover.length; ++i) {
    if (line_hover[i]) {
      line_hover[i] = false;
...
//If mouse is clicked, check specifically if a column or one of the buttons was clicked
void mouseClicked() {
  for (int i = 0; i < values[0].length; ++i) {
